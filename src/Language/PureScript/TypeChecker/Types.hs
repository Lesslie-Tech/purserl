...
lookupTypeClass :: Qualified (ProperName 'ClassName) -> Check TypeClassData
...
typesOf
  ...
  -> Check [((SourceAnn, Ident), (Expr, SourceType))]
...
instantiateConstraint :: Expr -> Type SourceAnn -> Check (Expr, Type SourceAnn)
...
insertUnkName' :: SourceType -> Text -> Check ()
...
inferProperties
  :: [(PSString, Expr)]
  -> Check [(PSString, (Expr, SourceType))]
...
inferLetBinding
  ...
  -> [Declaration]
  -> Expr
  -> (Expr -> Check TypedValue')
  -> Check ([Declaration], TypedValue')
...
inferBinder
  :: SourceType
  -> Binder
  -> Check (M.Map Ident (SourceSpan, SourceType))
...
inferBinder val (LiteralBinder _ (ObjectLiteral props)) = do
  ...
  return m1
  where
  inferRowProperties :: SourceType -> SourceType -> [(PSString, Binder)] -> Check (M.Map Ident (SourceSpan, SourceType))
...
--
ensureNoDuplicateProperties :: [(PSString, Expr)] -> Check ()
ensureNoDuplicateProperties ps =
