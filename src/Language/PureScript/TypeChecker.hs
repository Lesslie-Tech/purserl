...
addTypeClass _ qualifiedClassName args implies dependencies ds kind = do
  ...
  where
    ...
    mkNewClass :: Check TypeClassData
    mkNewClass = do
      env <- getEnv
...
typeCheckAll moduleName = traverse go
  ...
  checkInstanceArity :: Ident -> Qualified (ProperName 'ClassName) -> TypeClassData -> [SourceType] -> Check ()
  ...
  replaceTypeSynonymsInDataConstructor :: DataConstructorDeclaration -> Check DataConstructorDeclaration
...
typeCheckModule modulesExports (Module ss coms mn decls (Just exps)) =
  ...
  checkMemberExport :: (SourceType -> [DeclarationRef]) -> DeclarationRef -> Check ()
  ...
  checkExport :: DeclarationRef -> [DeclarationRef] -> Check ()
  ...
  checkDataConstructorsAreExported :: DeclarationRef -> Check ()
